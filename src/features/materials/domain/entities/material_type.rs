use crate::features::materials::domain::{errors::MaterialError, value_objects::MaterialTypeId};
...
impl MaterialType {
    ...
    pub fn create(name_ru: String, name_en: String) -> Result<Self, MaterialError> {
        if name_ru.trim().is_empty() {
            return Err(MaterialError::ValidationError { message: "Название типа материала (RU) не может быть пустым".to_string() });
        }
        if name_en.trim().is_empty() {
            return Err(MaterialError::ValidationError { message: "Название типа материала (EN) не может быть пустым".to_string() });
        }

        Ok(Self {
            id: MaterialTypeId::new(0)?, // Временный ID, будет заменен после сохранения
            name_ru: name_ru.trim().to_string(),
            name_en: name_en.trim().to_string(),
        })
    }
    ...
    pub fn with_id(mut self, id: MaterialTypeId) -> Self {
        self.id = id;
        self
    }
}
