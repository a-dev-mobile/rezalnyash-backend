...
<svg width="{}" height="{}" viewBox="0 0 {} {}" xmlns="http://www.w3.org/2000/svg">"#,
    ...
    .margin-indicator {{ stroke: #888888; stroke-width: 1; stroke-dasharray: 5,5; }}
...
<rect class="sheet" x="{}" y="{}" width="{}" height="{}" />"#,
    ...
    /// Добавляет индикаторы отступов
    pub fn add_margin_indicators(&mut self, x: u32, y: u32, width: u32, height: u32) -> &mut Self {
        writeln!(
            &mut self.content,
            r#"<!-- Индикаторы отступов -->
<rect class="margin-indicator" x="{}" y="{}" width="{}" height="{}" fill="none" />"#,
            x, y, width, height
        ).unwrap();
...
<g>"#,
        ...
        self.add_dimension_text(
            detail.x - 5, 
            detail.y + detail.length / 2, 
            &format!("{}мм", detail.length),
            "dimension"
        );
...
<text class="title" x="{}" y="{}">{}</text>"#,
    ...
    pub fn add_rect(&mut self, x: u32, y: u32, width: u32, height: u32, class: &str, extra_attrs: Option<&str>) -> &mut Self {
        match extra_attrs {
            Some(attrs) => writeln!(
                &mut self.content,
